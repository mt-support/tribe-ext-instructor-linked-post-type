...
class Tribe__Events__Filterbar__Filters__Instructor extends Tribe__Events__Filterbar__Filter {
	...
	protected function get_values() {
		...
		$instructor_ids = $wpdb->get_col( $wpdb->prepare( "SELECT DISTINCT m.meta_value FROM {$wpdb->postmeta} m INNER JOIN {$wpdb->posts} p ON p.ID=m.post_id WHERE p.post_type=%s AND p.post_status='publish' AND m.meta_key=%s AND m.meta_value > 0",
	...
	protected function setup_join_clause() {
		global $wpdb;
		$this->joinClause = $wpdb->prepare( "INNER JOIN {$wpdb->postmeta} AS instructor_filter ON ({$wpdb->posts}.ID = instructor_filter.post_id AND instructor_filter.meta_key=%s)",
			Tribe__Extension__Instructor_Linked_Post_Type::instance()->get_linked_post_type_custom_field_key()
		);
	}
